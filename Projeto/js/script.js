...
function preencherPrecoTotal(quantidade){
    if(quantidade.value != "" && document.getElementById("precoUnid").value != ""){
        ...
        document.getElementById("precoTotal").value = nQuantidade*valorUnitario;
    }
    if(quantidade.value == ""){
...
function cadastrarComprador(){
    ...
    if(verificaCamposCadastro()){
        fetch('http://localhost:3000/Comprador',{
            method:'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
function swalCadastroComprador(){
    if(sessionStorage.getItem('novoId') != "undefined"){
        Swal.fire({
            icon: 'success',
...
function exibirTabelaCompradores(){
    ...
    .then(response => 
        ...
        <td>${compradores[x].email}<a href="mailto:email@provedor.com.br"><img class="imgWhats" src="/Projeto/public/imagens/email.png" class="icoEmail"></a></td>
        <td>${compradores[x].telefone}<a href="https://api.whatsapp.com/send?1=pt_BR&phone=55${compradores[x].telefone}" target="_blank"><img class="imgWhats" src="/Projeto/public/imagens/whatsapp.png" class="icoWhats"></a></td>
        </tr>`
        document.getElementById("tbody").innerHTML += linha
        }
    })
    ...
    .then(response => 
        ...
        if(comprador.nome == undefined){
            ...
            if(comprador.senha == document.getElementById("senha").value){
                ...
                window.location='/Projeto/public/perfil.html';
            }else{
...
function senhaGestor(){
    Swal.fire({
        ...
        preConfirm: (senha) => {
            if(senha == 123456){
                window.location='/Projeto/public/areaGestor.html';
            }else{
                Swal.fire({
